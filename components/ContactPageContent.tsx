...
export default function ContactPageContent({page, siteSettings}: ContactPageProps) {
      ...
      <section className="white">
        <div className="container">
          <div className="contact-grid">
            <div>
              <h2 className="section-title">{page.detailsTitle}</h2>
              ...
              <div className="contact-info-item">
                ...
                <div>
                  <div className="ci-label">{page.contactPersonLabel}</div>
                  <div className="ci-value">{page.contactPersonValue}</div>
                </div>
              </div>
              ...
              <div className="contact-info-item">
                ...
                <div>
                  <div className="ci-label">{page.addressLabel}</div>
                  <div className="ci-value" style={{whiteSpace: 'pre-line'}}>{page.addressValue}</div>
                </div>
              </div>
              ...
              <div className="contact-info-item">
                ...
                <div>
                  <div className="ci-label">{page.phoneLabel}</div>
                  <div className="ci-value"><a href={`tel:${page.phoneValue.replace(/\s+/g, '')}`}>{page.phoneValue}</a></div>
                </div>
              </div>
              ...
              <div className="contact-info-item">
                ...
                <div>
                  <div className="ci-label">{page.emailLabel}</div>
                  <div className="ci-value"><a href={`mailto:${page.emailValue}`}>{page.emailValue}</a></div>
                </div>
              </div>
              ...
              <div className="contact-info-item">
                ...
                <div>
                  <div className="ci-label">{page.webLabel}</div>
                  <div className="ci-value"><a href={page.webValue} target="_blank" rel="noopener noreferrer">{page.webValue}</a></div>
                </div>
              </div>
              ...
              <div style={{background: 'var(--bg)', borderRadius: 12, padding: '1.5rem', border: '1px solid var(--border)', marginTop: '0.5rem'}}>
                <div style={{fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--text-muted)', marginBottom: '0.75rem'}}>
                  {page.companyDataTitle}
                </div>
                <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem'}}>
                  <div style={{display: 'flex', justifyContent: 'space-between'}}><span style={{color: 'var(--text-muted)'}}>IČO</span><strong>{page.companyIco}</strong></div>
                  <div style={{display: 'flex', justifyContent: 'space-between'}}><span style={{color: 'var(--text-muted)'}}>DIČ</span><strong>{page.companyDic}</strong></div>
                  <div style={{display: 'flex', justifyContent: 'space-between'}}><span style={{color: 'var(--text-muted)'}}>Zápis</span><strong>{page.companyYear}</strong></div>
                </div>
              </div>
            </div>
            ...
            <div className="contact-form-wrap">
              ...
              <div className="form-group">
                <label>{page.formMessageLabel}</label>
